...
static const Query_options options = {
    0 /* query_spec_options */, SELECT_LEX::SQL_CACHE_UNSPECIFIED /* sql_cache */
};
...
static bool add_expression(const POS &pos, THD *thd, PT_select_item_list *item_list, const LEX_STRING &field,
...
bool build_processlist_query(const POS &pos, THD *thd, bool verbose)
{
  ...
  PT_select_item_list *item_list_star = new (thd->mem_root) PT_select_item_list();
  if (item_list_star == NULL)
    return true;
  ...
  PT_select_options_and_item_list *select_options_and_item_list =
      new (thd->mem_root) PT_select_options_and_item_list(options, item_list_star);
  if (select_options_and_item_list == NULL)
    return true;
  ...
  */
  ...
  if (verbose)
  {
    if (!thd->make_lex_string(&info_len, "1024", 4, false))
      return true;
  }
  else
  {
    if (!thd->make_lex_string(&info_len, "100", 3, false))
      return true;
  }
  ...
  if (add_expression(pos, thd, item_list, field_id, alias_id))
    return true;
  if (add_expression(pos, thd, item_list, field_user, alias_user))
    return true;
  if (add_expression(pos, thd, item_list, field_host, alias_host))
    return true;
  if (add_expression(pos, thd, item_list, field_db, alias_db))
    return true;
  if (add_expression(pos, thd, item_list, field_command, alias_command))
    return true;
  if (add_expression(pos, thd, item_list, field_time, alias_time))
    return true;
  if (add_expression(pos, thd, item_list, field_state, alias_state))
    return true;

  /* ... INFO ... */
  PTI_simple_ident_ident *ident_info = new (thd->mem_root) PTI_simple_ident_ident(pos, field_info);
  if (ident_info == NULL)
    return true;

  /* Info length is either "100" or "1024" depending on verbose */
  Item_int *item_info_len = new (thd->mem_root) PTI_num_literal_num(pos, info_len);
  if (item_info_len == NULL)
    return true;

  /* ... LEFT(INFO, <info_len>) AS Info ...*/
  Item_func_left *func_left = new (thd->mem_root) Item_func_left(pos, ident_info, item_info_len);
  if (func_left == NULL)
    return true;

  PTI_expr_with_alias *expr_left = new (thd->mem_root) PTI_expr_with_alias(pos, func_left, pos.cpp, alias_info);
  if (expr_left == NULL)
    return true;
  ...
  */
  ...
  if (!thd->make_lex_string(&tmp_table_processlist, table_processlist.str, table_processlist.length, false))
    return true;

  /* ... performance_schema.processlist ... */
  Table_ident *table_ident_processlist = new (thd->mem_root) Table_ident(tmp_db_name, tmp_table_processlist);
  if (table_ident_processlist == NULL)
    return true;

  /* ... FROM performance_schema.processlist ... */
  PT_table_factor_table_ident *table_factor_processlist =
      new (thd->mem_root) PT_table_factor_table_ident(table_ident_processlist, NULL, NULL, NULL);
  if (table_factor_processlist == NULL)
    return true;

  PT_join_table_list *join_table_list_1 = new (thd->mem_root) PT_join_table_list(pos, table_factor_processlist);
  if (join_table_list_1 == NULL)
    return true;

  PT_table_reference_list *table_reference_list_1 = new (thd->mem_root) PT_table_reference_list(join_table_list_1);
  if (table_reference_list_1 == NULL)
    return true;

  PT_table_expression *table_expression =
      new (thd->mem_root) PT_table_expression(table_reference_list_1, NULL, NULL, NULL, NULL, NULL, NULL, lock_type);
  ...
  if (table_factor_nested == NULL)
    return true;

  PT_select_derived *derived = new (thd->mem_root) PT_select_derived(pos, table_factor_nested);
  if (derived == NULL)
    return true;

  PT_select_derived_union_select *derived_union =
      new (thd->mem_root) PT_select_derived_union_select(derived, NULL, pos);
  if (derived_union == NULL)
    return true;

  /* ... derived_table ... */
  LEX_STRING derived_table_name;
  if (!thd->make_lex_string(&derived_table_name, table_processlist.str, table_processlist.length, false))
    return true;

  PT_table_factor_parenthesis *table_factor_2 =
      new (thd->mem_root) PT_table_factor_parenthesis(derived_union, &derived_table_name, pos);
  if (table_factor_2 == NULL)
    return true;

  PT_join_table_list *join_table_list_2 = new (thd->mem_root) PT_join_table_list(pos, table_factor_2);
  if (join_table_list_2 == NULL)
    return true;

  PT_table_reference_list *table_reference_list_2 = new (thd->mem_root) PT_table_reference_list(join_table_list_2);
  if (table_reference_list_2 == NULL)
    return true;

  PT_select_part2 *select_part2;
  select_part2 = new (thd->mem_root) PT_select_part2(select_options_and_item_list, /* select_options_and_item_list */
                                                     NULL,                         /* opt_into */
  ...
  PT_select_init2 *select_init2 = new (thd->mem_root) PT_select_init2(NULL, select_part2, NULL);
  if (select_init2 == NULL)
    return true;

  PT_select *select = new (thd->mem_root) PT_select(select_init2, SQLCOM_SELECT);
  if (select == NULL)
    return true;
