...
class kdf_argument
{
 ...
 public:
  kdf_argument() : tmp_option_value(tmp_option_buff, sizeof(tmp_option_buff), system_charset_info)
  {
    memset(tmp_option_buff, '\0', max_kdf_option_size);
  }

  bool parse_kdf_option(String *kdf_option_value, string &kdf_option, my_bool *error_generated,
  ...
  */
  vector< string > retrieve_kdf_options(uint arg_count, Item **args, const char *func_name, my_bool *error_generated)
  {
    ...
    *error_generated = FALSE;
    ...
    if (!parse_kdf_option(kdf_option_value, kdf_option, error_generated, max_size_allowed))
      return kdf_options;
    kdf_options.push_back(kdf_option);
...
String *Item_func_aes_encrypt::val_str(String *str)
{
  ...
  if (sptr && key)  // we need both arguments to be not NULL
  {
    ...
    kdf_options = kdf_arg.retrieve_kdf_options(arg_count, args, func_name(), &null_value);
    if (null_value)
    {
      DBUG_RETURN(NULL);
    }
    ...
    {
      push_warning_printf(thd, Sql_condition::SL_WARNING, WARN_AES_KEY_SIZE, ER_THD(thd, WARN_AES_KEY_SIZE), rkey_size);
    }
    if (!str_value.alloc(aes_length))  // Ensure that memory is free
    {
      // finally encrypt directly to allocated buffer.
      if (my_aes_encrypt((unsigned char *)sptr->ptr(), sptr->length(), (unsigned char *)str_value.ptr(),
                         (unsigned char *)key->ptr(), key->length(), (enum my_aes_opmode)aes_opmode, iv_str, true,
                         (kdf_options.size() > 0) ? &kdf_options : NULL) == aes_length)
      {
