...
#define framep_trace_flag(cs, frp) \
  (frp ? frp->level & TRACE_ON : (ListFlags(cs->stack->functions) & INCLUDE) ? 0 : (uint)TRACE_ON)

void FixTraceFlags_helper(CODE_STATE *cs, const char *func, struct _db_stack_frame_ *framep)
{
  if (framep->prev)
...
int _db_explain_(CODE_STATE *cs, char *buf, size_t len)
{
  ...
  op_str_to_buf(((cs->stack->flags & FLUSH_ON_WRITE ? 0 : 32) | (cs->stack->flags & OPEN_APPEND ? 'A' : 'O')),
...
static struct link *ListAddDel(struct link *head, const char *ctlp, const char *end, int todo)
{
  const char *start;
  struct link **cur;
  size_t len;
...
static int DoTrace(CODE_STATE *cs)
{
  if (0 == strcmp("alloc_root", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("free_root", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("my_malloc", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("my_free", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("mysql_fetch_row", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("can_be_evaluated_now", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("row_sel_store_mysql_field_func", cs->func))
  {
    return DONT_TRACE;
  }

  if (0 == strcmp("row_sel_store_mysql_rec", cs->func))
  {
    return DONT_TRACE;
  }
...
BOOLEAN _db_keyword_(CODE_STATE *cs, const char *keyword, int strict)
{
  ...
  result = DoTrace(cs) & DO_TRACE && InList(cs->stack->keywords, keyword) & strict;
...
static void DBUGOpenFile(CODE_STATE *cs, const char *name, const char *end, int append)
{
  FILE *fp;
